  ...
  return (
    <tr className="group flex flex-col mx-auto max-w-md w-full bg-base-100 rounded-2xl shadow-sm border border-base-200 p-4 mb-4 md:mb-0 md:table-row md:max-w-none md:bg-transparent md:border-0 md:border-b md:border-base-200 md:shadow-none md:rounded-none md:p-0 hover:bg-base-200/40 transition-colors cursor-pointer last:border-0">
          ...
          <div className="avatar shrink-0">
            <div className="bg-base-200/80 text-base-content/60 rounded-full w-10 h-10 border border-base-200/50 flex items-center justify-center overflow-hidden">
              {appliance.image && appliance.image.startsWith('http') ? (
                <img src={appliance.image} alt={localizeApplianceName(appliance.name, t)} className="w-full h-full object-cover" />
              ) : (
                <span className="text-base" role="img" aria-label={localizeApplianceName(appliance.name, t)}>{appliance.image || "🔌"}</span>
              )}
            </div>
          </div>
          <div className="min-w-0 flex-1">
            <div className="font-bold text-base-content text-[14px] leading-tight truncate">{localizeApplianceName(appliance.name, t)}</div>
            <div className="text-xs text-base-content/50 mt-0.5 truncate">{appliance.location ? localizeRoomName(appliance.location, t) : '-'}</div>
          </div>
        </div>
      ...
      <td className="flex justify-between items-center p-0 border-0 md:px-4 md:py-3 md:table-cell w-full md:w-auto pt-3 border-t border-base-200 md:border-0 md:pt-3 mt-2 md:mt-0 align-middle">
        <span className="text-xs font-semibold text-base-content/50 md:hidden">{t('appliances.usage')}</span>
      ...
      <td className="flex justify-between items-center p-0 border-0 md:px-4 md:py-3 md:table-cell w-full md:w-auto pt-3 md:pt-3 align-middle md:text-center">
        <span className="text-xs font-semibold text-base-content/50 md:hidden">{t('appliances.energy')}</span>
      ...
      <td className="flex justify-between items-center p-0 border-0 md:px-4 md:py-3 md:table-cell w-full md:w-auto pt-3 md:pt-3 align-middle md:text-right md:pr-4">
        <span className="text-xs font-semibold text-base-content/50 md:hidden">{t('appliances.estimatedCostBaht')}</span>
